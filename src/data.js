...
sensors = sensors.map((sensor, i) => ({ id: i, ...sensor, fake: true }))

let liveSensors = []

function getData() {
  let allSensors = sensors.concat(liveSensors)
  return allSensors.map( (poly, i) => {
    const [ lat, lng ] = poly.coords
    let devices
	  if (!poly.fake) {
      devices = poly.devices
    } else {
      devices = 250*Math.sin((new Date().getTime()+i*1000232^2324321)/(60*0.3*1000)) + 250
    }
	  return { id: poly.id, devices, lat, lng, time: new Date() }
	})
} 
